...
/// `true`/`false` cuando ya se leyó el onboarding; `null` mientras se carga.
///
/// El router espera a que deje de ser `null` antes de decidir entre el
/// onboarding y el catálogo público, evitando una carrera con el arranque.
final onboardingDoneProvider = StateProvider<bool?>((ref) => null);

/// Lee el estado del onboarding y lo publica para el router.
Future<bool> loadOnboardingDone(WidgetRef ref) async {
  final done = await ref.read(tokenStoreProvider).isOnboardingDone();
