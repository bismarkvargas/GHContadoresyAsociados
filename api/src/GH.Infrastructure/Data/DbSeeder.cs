...
public static class DbSeeder
{
    ...
    private static async Task SeedSettingsAsync(GhDbContext db, CancellationToken ct)
    {
        var defaults = new (string Key, string Value, string Group, string Description)[]
        {
            ...
            ("brand.primaryColor", "#DF3131", "Marca", "Rojo corporativo (extraído de ghcontadores.net)"),
            ("brand.inkColor", "#212121", "Marca", "Color de estructura y texto principal"),
            ("brand.accentColor", "#E62214", "Marca", "Color de acción"),
            ("brand.successColor", "#008250", "Marca", "Color de éxito"),
            ("brand.logoUrl", "", "Marca", "URL pública del logotipo"),
    ...
    private static async Task SeedDemoClientAsync(GhDbContext db, ILogger logger, CancellationToken ct)
    {
        ...
        {
            Code = "GH-EXP-" + DateTime.UtcNow.Year + "-0001",
            ClientId = client.Id,
        ...
        {
            Code = "GH-EXP-" + DateTime.UtcNow.Year + "-0002",
            ClientId = client.Id,
        ...
        if (product is not null)
        {
            var order = new Order
            {
                Number = "GH-ORD-" + DateTime.UtcNow.Year + "-00001",
                UserId = user.Id,
                ClientId = client.Id,
        ...
        db.AccountRequests.Add(new AccountRequest
        {
            ...
            TrackingCode = "GH-SOL-260101-0001",
